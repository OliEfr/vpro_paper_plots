r"""Latent-space embodiment mixing at 2 and 5 embodiments, start versus end -- t-SNE.

Reads ``results/tsne_teachers.csv`` and emits one SINGLE-COLUMN 2x2 figure:

    python plot_tsne_teachers_2x2.py   # figures/tsne_teachers_2x2.pdf

              1k steps        70k steps
    2 emb   [ xemb1iiwa ]   [ xemb1iiwa ]
    5 emb   [ xemb4all  ]   [ xemb4all  ]

This is the t-SNE twin of plot_umap_teachers_2x2.py: same teachers, same
checkpoints, same balanced sample off the same seed, same page width. The two
differ only in the projector, so a referee asking whether the neighbour-graph
embedder manufactured the effect can be answered by putting them side by side.
See ``experiments/fit_tsne_dumps.py``.

WHY THIS ONE MAY BE THE BETTER FIGURE OF THE PAIR
-------------------------------------------------
The figure exists to show a starting separation that later disappears, so the
1k row has to actually show the separation. Chance-normalised 2-D neighbourhood
purity -- the fraction of a point's 15 nearest neighbours IN THE PROJECTION
sharing its label, which is what a reader's eye integrates -- against the 8-D
kNN number the paper reports:

                       ckpt    8-D kNN    t-SNE 2-D    UMAP 2-D
    xemb1iiwa (2 emb)    1k      0.774        0.699       0.603
                        70k      0.107        0.014       0.011
    xemb4all  (5 emb)    1k      0.622        0.474       0.334
                        70k      0.122        0.038       0.017

Both projectors reproduce the collapse and both preserve the 2 > 3 > 5 ordering
at 1k, so nothing about the claim depends on the choice. But UMAP compresses the
1k panels into dense clumps whose colours already read as mixed, losing 0.10-0.14
of purity relative to t-SNE and underselling the very thing the top row is for.

BOTH PROJECTIONS UNDERSHOOT AT 70k, and a caption must not paper over it: the
8-D residual is gap-closed 0.107 and 0.122, but neither picture shows more than
0.04. Embodiment is not gone at 70k, it is gone FROM THE PROJECTION. If the text
says "indistinguishable" the number contradicts it; "no longer visible" is the
claim the figure supports.

THE DECODABILITY NUMBERS ARE NOT REFITTED FOR t-SNE. They are computed on the
raw 8-D latent, never on a projection, so ``umap_decodability.csv`` is the right
table for this figure too despite the name.

READING A t-SNE PANEL -- WHAT IS AND IS NOT INFORMATION
-------------------------------------------------------
t-SNE preserves local neighbourhoods. It does not preserve inter-cluster
distance and it does not preserve cluster size: the gap between two blobs and
the area of a blob are artefacts of the perplexity and the optimisation, not
measurements. What is readable is whether points of different colour are
neighbours -- exactly and only the claim this figure makes.

SIZE, AND WHY IT IS THE WHOLE POINT
-----------------------------------
Every figure in this repo is drawn with 8pt text and included at 1:1, so 8pt
text lands on the page at 8pt. That contract only holds if the PDF is authored
at the width it will be included at. tsne_teachers.pdf is a \textwidth grid;
dropped into a \columnwidth slot LaTeX rescales it by 251/514 = 0.488 and its
text arrives at 3.6pt, unreadable in print. This file is authored at
style.COL_WIDTH and nothing rescales it.

WHY THE AXES ARE BARE. A t-SNE coordinate is not a measurement -- no units, no
origin, no meaningful scale, axes not independent. Tick VALUES would invite
reading distances off the page, which the method does not support. Axis NAMES
carry no information the caption does not, and at \columnwidth across four
panels they cost real drawing area. Both omitted.

COORDINATES ARE PER TEACHER. The two rows are separate fits of two separately
trained latent spaces, so nothing about a position in the top row relates to a
position in the bottom one. Within a row the fit is joint across checkpoints,
but each panel is still cropped to its own points: training moves the manifold,
so at the shared extent each checkpoint shrinks into a corner and the mixing --
the thing being shown -- stops being visible. What is comparable between panels
is the degree of mixing, never the location.
...
------------------
    \begin{figure}[t]
      \centering
      \includegraphics[width=\columnwidth]{figures/tsne_teachers_2x2.pdf}
      \caption{...}
    \end{figure}
"""
import argparse
...
# Robot -> colour, fixed by position and shared with every figure in this
# family. Positions 0-3 are the style module's PALETTE unchanged; the fifth is
# Okabe-Ito black. Worst-pair dE (CIE76, Vienot dichromat simulation) over ALL
# pairs:
#
...
def main():
    ...
    build(df, f"tsne_teachers_2x2{suffix}", plt)
