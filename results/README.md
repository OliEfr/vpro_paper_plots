...
**`mv_sf` and `mv_mf` are exact, protocol-identical to the LIBERO-PLUS and
MimicGen bands**: the same `analyze_latent_feature_distribution.py` (md5
`8c97d328…`, same flags: MLP+ridge, episode split, continuous features,
`--future-frames 5`, `--probe-max-samples 0`, seed 42) run on MN5 against the
paper teachers' own whole-pool label exports, selector
mlp/episode/continuous/`current_action` — exactly the selector the
LIBERO-PLUS commit (811d7e0) verified against the MimicGen gate dirs.
`mv_mf` = lam1 (2×[0,1,5,9], job 42532997, ckpt070000, idx1 export; probe
job 44965976, 4.58 M valid rows, GATE_OK). `mv_sf` = lam1o5_lib90t71
(2×[0,5], job 44161039, ckpt070000, idx0 export; probe job 44965977, 4.71 M
valid rows, GATE_OK). Note the two are equal within the ±0.011 probe noise
floor (means 0.6724 vs 0.6728) — the same sf≈mf pattern MimicGen shows
(0.558 vs 0.5606).
...
level against the exact `mv_*` columns. `sv_mf` is 0 = not run anywhere.
