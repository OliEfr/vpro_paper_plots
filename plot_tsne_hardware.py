r"""Real-hardware latent: robot demonstrations versus human video -- t-SNE.

Reads ``results/tsne_hardware.csv`` and emits:

    python plot_tsne_hardware.py   # figures/tsne_hardware.pdf
                                   # figures/tsne_hardware_sharedlam2.pdf

The t-SNE counterpart of ``plot_umap_hardware.py``, existing for the reason
``plot_tsne_teachers.py`` does: the contrast between simulation and hardware is
a claim about the data, and it should not rest on one embedder. Same latents,
same balanced sample, same seed, same layout -- only the projector differs
(``experiments/fit_tsne_dumps.py``).

THE POINT OF THIS FIGURE IS THAT IT DOES NOT LOOK LIKE THE SIMULATION ONE. In
LIBERO the robots start separable and end interleaved (plot_tsne_teachers.py).
...
LAMs, and never falls (results/umap_decodability.csv -- fitted on the raw 8-D
latent, so it applies to this figure too despite the name).
...
slightly later point; ``tsne_hardware_sharedlam2`` uses the real 1k and is the
...
READING A t-SNE PANEL. t-SNE preserves local neighbourhoods; it preserves
neither inter-cluster distance nor cluster size. The width of the gap between
the robot and video blobs is therefore NOT a measure of how different the two
sources are -- only the fact that points of one colour have neighbours of one
colour is. The separation claim rests on that, and on the decodability numbers,
never on the gap. A caption that invites the reader to measure the gap
overclaims what the method delivers.
...
DEFAULT_CSV = Path(__file__).resolve().parent / "results" / "tsne_hardware.csv"

# Two sources, two colours, identical to plot_umap_hardware.py so the two
# projections can be read against each other: palette positions 0 and 2
# (blue / orange), the pair with the widest separation under both dichromat
# simulations of the four.
SOURCE_ORDER = ["robot_3cam", "video_2cam"]
SOURCE_COLORS = {}  # filled in main(), after the --style swap (see plot_tsne_teachers.py)
...
def main():
    ...
    build_grid(df, f"tsne_hardware{suffix}", plt)
    build_single(df, "sharedlam2", f"tsne_hardware_sharedlam2{suffix}", plt)
